...
const EditRequiredDocumentModal: React.FC<any> = ({
  defaultValue,
  closeModal,
}) => {
  ...
  const onSubmit = async (data: TRequiredDocument) => {
    try {
      await API.updateRequiredDocument(defaultValue.id, {
        title: data.title,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="mb-4">
        ...
        {errors.description && (
          <div role="alert" className="flex items-center mt-2 text-red">
            <Icons.required />
            <p className="text-sm ml-1 mt-[2px]">
      ...
      <label htmlFor="isRepeated">
        <input
          type="checkbox"
          disabled={watch("documentType") === "QUESTION"}
          name="isRepeated"
          id="isRepeated"
          checked={isRepeated}
