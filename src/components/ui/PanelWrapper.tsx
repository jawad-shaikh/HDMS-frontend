                                        ...
                                        <div className="relative flex-1 px-4 sm:px-6">
                                            ...
                                            <dd className="mt-2 text-sm text-[#616161] sm:col-span-2 sm:mt-0 border border-gray mb-4">
                                                <div className="flex items-center justify-between p-4 border-b border-gray">
                                                    ...
                                                    {
                                                        ((document?.documents?.length && !document?.documents[0].hasDownloaded) && (user.role === "ADMIN" || user.role === "HR")) && <button onClick={() => downloadDoc(document.id)} className="border border-primary text-primary font-medium text-xs px-4 py-2"> Download All File(s)
                                                        </button>
                                                    }
                                                </div>
                                                ...
                                                <ul role="list" className="divide-y divide-white">
                                                    {document?.documents &&
                                                        (document.documents.map((document: any) => (
                                                            <li key={document.fileName} className="flex items-center justify-between py-4 pl-4 pr-5 text-sm leading-6">
                                                                <div className="flex w-0 flex-1 items-center">
                                                                    <Icons.link className="h-5 w-5 flex-shrink-0 text-gray-400" aria-hidden="true" />
                                                                    <div className="ml-4 flex min-w-0 flex-1 gap-2">
                                                                        <span className="truncate font-medium">{document.fileName}</span>
                                                                    </div>
                                                                </div>
                                                                <div className="ml-4 flex-shrink-0">
                                                                    <span className="flex-shrink-0 text-gray-400">{document.size / 1000} mb</span>
                                                                </div>
                                                            </li>)
                                                        ))}
                                                </ul>
                                            ...
                                            } */}
                                            {
                                                document.documentRequest && <div className="border-t border-gray">
                                                    <div className='my-4'>
                                                        ...
                                                        <input type='input' className='block border-2 border-gray w-full p-2 outline-none' disabled value={document.documentRequest.documentType} />
                                                    </div>
                                                    <div className='mb-4'>
                                                        <label className='block mb-2 text-sm'>Description</label>
                                                        <textarea className='block border-2 border-gray w-full p-2 outline-none' rows={5} disabled value={document.documentRequest.description} />
                                                    </div>
                                                    <div className="grid grid-cols-2 gap-4">
                                                        <div className='mb-4'>
                                                            <label className='block mb-2 text-sm'>Upload Date</label>
                                                            <input type='input' className='block border-2 border-gray w-full p-2 outline-none' disabled value={convertDate(document.documentRequest.updatedAt)} />
                                                        </div>
                                                        <div className='mb-4'>
                                            ...
                                            <details className="group [&_summary::-webkit-details-marker]:hidden border-t pt-4 border-gray" open>
                                                <summary
